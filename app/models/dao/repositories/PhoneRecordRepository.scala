...
class PhoneRecordRepositoryImpl extends PhoneRecordRepository {
  ...
  override def findWithAddress2(phone: String): Option[(PhoneRecord, Option[Address])] = transaction {
    join(phoneRecords, addresses.leftOuter)((r, a) =>
      where(r.phone === phone)
        select (r, a) // select r.*, a.* from Record as r
        orderBy(r.phone desc)
        on (Some(r.addressId) === a.map(_.id))
    ).page(0, 1).headOption
  }

  override def list(): List[PhoneRecord] =
    transaction(from(phoneRecords)(r => select(r)).toList)
  ...
  override def insert(phoneRecord: PhoneRecord): Unit = transaction {
    phoneRecords.insert(phoneRecord)
  }

  override def insert(address: Address): Unit = transaction {
